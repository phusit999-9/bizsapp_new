...
class MyPDF extends TCPDF {
    ...
    {   
       ...
       // $customer_state_name = (!empty($state)) ? $state->state : $store->state;
        ...
        $custmer_details .= "<br>".$this->CI->lang->line('name')." : ".$customer->customer_name;
        $custmer_details .= "<br>".$this->CI->lang->line('address')." : ".$customer->address;
        $custmer_details .= "<br>".$this->CI->lang->line('phone')." : ".$customer->phone;
        $custmer_details .= "<br>".$this->CI->lang->line('mobile')." : ".$customer->mobile;
        $custmer_details .= "<br>".$this->CI->lang->line('email')." : ".$customer->email;
        $custmer_details .= "<br>".$this->CI->lang->line('gst_number')." : ".$customer->gst_no; 
        $custmer_details .= "<br><b>".$this->CI->lang->line('tax_number')." :</b> ".$customer->vat_no; 
        $this->writeHTMLCell($w, $h, $x ='6', $y='40', $custmer_details, 1, 0, 1, true, 'J', true);
        return $this;
    } 
    ...
    {   
        ...
        */
        ...
        $lang_array['English'] = 'thsarabun_b';

        $lang_array['Thai'] = 'thsarabun_b';

         if(strtoupper($this->_app_language)==strtoupper('Thai')){
           return $lang_array['Thai'];
        }
    ...
    {
        // Set font
        $this->setFont($this->get_font_name(), '', 18);
      
        // Title
        $this->writeHTMLCell(0, 50, $x ='', $y='8', strtoupper($this->_invoice_name), $border = 0, 1, 0, true, 'C', true);
      //  $this->writeHTMLCell(0, 50, $x ='', $y='8', $this->CI->lang->line('invoice_pos'), $border = 0, 1, 0, true, 'C', true);
        $this->Ln();
        return $this;
    }
    ...
    {   
        ...
        $this->Image($image_file, $x = 20, $y = 10, 25, '', '', '', 'T', false, 300, '', false, false, $border =0, false, false, false);
        return $this;
    }
    ...
    {
        $store = $this->store;
        $txt='<div style="line-height: 1;">';
        $txt .= '<span style="font-size:22px; ">'.$store->store_name.'</span>';
        $txt .= '<br><span style="font-size:18px;">'.$store->address.'</span>';
      //  $txt .= '<br><span style="font-size:16px;"><b>';
        $txt .= '<br><span style="font-size:18px;"><b>';

         if(!empty($store->gst_no)){
            $txt .= $this->CI->lang->line('tax_id').' : </b> '.$store->gst_no; 
           }
        if(!empty($store->vat_no)){
                $txt .= $this->CI->lang->line('branch_no').' : </b> '.$store->pan_no; 
            }

        $txt .= '<br><span><b>'.$this->CI->lang->line('phone').': </b>'.$store->mobile.'<b> '.$this->CI->lang->line('email').':</b> '.$store->email.'</span>';         
        $txt .= '</div>'  ;      
        $this->setFont($this->get_font_name(), '', 16, '', true);
        $this->writeHTMLCell($w =140, 0, $x='70', $y='15', $txt, $border = 0, 0, 0, true, '', true);
        return $this;
    }
   ...
   public function _get_qr()
    ...
    public function _get_hr()
    {
        ...
        $this->MultiCell('', '', $txt ='', $border =0, 'L', 1, 1, $x = '', $y = '45', true,1, true);
        return $this;
    }
    ...
    public function _document_details()
    {
        ...
        $this->setAuthor('ScaleBizs');
        $this->setTitle($this->page_title);
        $this->setSubject('Invoice Details');
        $this->setKeywords('TCPDF, PDF/A, example, test, guide');
    }
